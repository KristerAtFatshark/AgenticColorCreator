...
- Build and unit tests are passing after the latest TreeView multi-select preview update.
...
- The latest TreeView multi-select preview update was validated by build and tests, but still needs manual visual confirmation in the running UI.
...
- Added Ctrl-click multi-select support to `CFTreeView`, with custom per-item selection state so multiple `CFTreeViewItem` rows can stay selected at the same time.
- Updated the MainWindow TreeView preview binding to consume a bindable `SelectedTreeViewItems` collection and show each selected item's `Text` and `Value` on separate lines in the preview field.
